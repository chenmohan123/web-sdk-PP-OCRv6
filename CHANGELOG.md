...
- 修复内置缓存清理后在途下载迟到写回的问题，按实际模型身份统计和清理缓存，等待 IndexedDB 事务提交后报告成功。
- Demo 清理时取消并等待任务、释放会话，展示当前模型与本 SDK 的缓存字节数，并恢复失败后的操作状态。
- 补齐五种接入示例的模型来源与生命周期处理；独立工程固定公开 npm 0.1.8，提供可安装、构建和运行的配置。

## 0.1.8 - 2026-08-26

- 为 npm 发布包新增中英双语 README，提供安装、快速开始、模型分发、完整文档、示例和在线 Demo 入口。
- 发布校验新增 README 必需文件检查，防止 npm 包页面再次缺少使用说明。

## 0.1.7 - 2026-08-25

- SDK 新增 `RuntimeOptions.onProgress`，报告清单、缓存、下载、完整性校验、模型加载和推理阶段；支持流式下载百分比并兼容无流式响应的浏览器。
- Demo 移除顶部状态栏，在左侧运行按钮下显示模型下载进度、模型加载、识别中、识别完成和稳定错误信息；右侧拆分模型下载与模型加载耗时。

## 0.1.6 - 2026-08-25

- 修复 CTC recognition 字典未包含 PaddleOCR `use_space_char` 追加的 ASCII 空格，确保字典条目数与模型输出类别数一致。
- Demo 中间图片改为固定视口内的单画布预览，支持适配、滚轮/按钮缩放、按钮拖动、空格临时拖动、Esc 退出和移动端触摸手势。
- 修复在图片视口滚轮缩放时页面外层滚动条跟随移动的问题。
