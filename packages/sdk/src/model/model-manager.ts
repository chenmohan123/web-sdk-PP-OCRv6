import type { CacheWriter, ModelCache, ModelCacheIdentity } from "../cache/model-cache";
...
export function createModelManager(options: { cache?: ModelCache; writeCache?: CacheWriter; fetchImpl?: typeof fetch; hash?: HashFunction; onProgress?: ProgressCallback; onSource?: (source: ProgressSource) => void } = {}): ModelManager {
  const cache = options.cache ?? createMemoryCache();
  return {
    cache,
    async load(request, signal) {
      const write = options.writeCache ?? cache.createWriter?.() ?? cache.set.bind(cache);
      if (signal?.aborted) throw new PPOCRv6Error("ABORTED", "模型加载已取消");
      ...
      if (signal?.aborted) throw new PPOCRv6Error("ABORTED", "模型加载已取消");
      await write(request, downloaded.bytes);
