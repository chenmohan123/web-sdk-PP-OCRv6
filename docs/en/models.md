...
ONNX files are excluded from npm. Versioned GitHub Release assets are verified by byte count and SHA-256 before use. Git LFS files are release sources; Hugging Face records provenance/fallback. Custom models must declare tensors, preprocessing, DB postprocessing or dictionary/decoder configuration; the SDK does not infer contracts from filenames.
